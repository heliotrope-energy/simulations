...
#TODO: fit modules in better
def calculate_irradiance(surface_tilt, surface_azimuth, albedo, wspd, drybulb, current_index, solpos, dhi, dni, ghi, module = sandia_modules['Canadian_Solar_CS5P_220M___2009_']):
    '''
    returns effective irradiance and the cell temperature.
    stops short of running SAPM, as that can be passed an array of data.
    '''
    ...
    return effective_irradiance, pvtemps.temp_cell
...
def calculate_energy(surface_tilt, surface_azimuth, albedo, wspd, drybulb, current_index, solpos, dhi, dni, ghi, tracker_name, save_data = True, module = sandia_modules['Canadian_Solar_CS5P_220M___2009_'],inverter = sapm_inverters['ABB__MICRO_0_25_I_OUTD_US_208_208V__CEC_2014_'] ):
    '''
    Energy calculations with logging.
    '''
    ...
    # sapm_out = pvlib.pvsystem.sapm(np.array([effective_irradiance,effective_irradiance]), np.array([pvtemps.temp_cell, pvtemps.temp_cell]), module)

    ac =  pvlib.pvsystem.snlinverter(sapm_out['v_mp'], sapm_out['p_mp'], inverter)

    return sapm_out['p_mp'], ac, rad_timestep, pvtemps
